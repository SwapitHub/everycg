 ...
 */
 ...
 public function getTagProducts($type = null, $limit = null, $opt = null, $sortBy = null, $sortOrder = 'desc', $tag=null)
    {
        $lang = sc_get_locale();
        $query = $this->where($this->getTable() . '.status', 1)->whereRaw('find_in_set('.$tag.',' .$this->getTable() . '.tags)')
            ->with(['descriptions' => function ($q) use ($lang) {
                $q->where('lang', $lang);
            }])
            ->with('promotionPrice');
     ...
     public function getFeatured($type = null, $limit = null, $opt = null, $sortBy = null, $sortOrder = 'desc')
    {
        $lang = sc_get_locale();
        $query = $this->where($this->getTable() . '.status', 1)->where($this->getTable() . '.featured', 1)
            ->with(['descriptions' => function ($q) use ($lang) {
                $q->where('lang', $lang);
            }])
            ->with('promotionPrice');
        ...
        return $this->where('status', 1)->with(['descriptions' => function ($q) use ($lang) {
            ...
            ->where(function ($sql) use ($keyword) {
                $sql->where((new ShopProductDescription)->getTable() . '.name', 'like', '%' . $keyword . '%')
                    ->orWhere($this->getTable() . '.sku', 'like', '%' . $keyword . '%')->orWhere($this->getTable() . '.tags', 'like', '%' . $keyword . '%');
            })
 ...
 */
    public function getProductsToCategory($category_id, $limit = null, $opt = null, $sortBy = null, $sortOrder = 'asc', $status = 1)
    {
        ...
        //Get all product
        if (!(int) $limit) {
            return $query->groupBy($this->getTable() . '.id')->get();
        } else
        //paginate
        if ($opt == 'paginate') {
            return $query->groupBy($this->getTable() . '.id')->paginate((int) $limit);
        } else
        //random
        if ($opt == 'random') {
            return $query->inRandomOrder()->groupBy($this->getTable() . '.id')->limit($limit)->get();
        }
        //
        else {
            return $query->groupBy($this->getTable() . '.id')->limit($limit)->get();
        }
 ...
 */
    ...
    public static function updateStock($product_id, $qty_change)
    {
     ...
     public static function relatedProduct($ids)
    {
        $response = (new ShopProduct)->where('status', 1)->whereIn('id', $ids)->get();
        return $response;
    }
}
