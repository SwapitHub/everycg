...
class UserOrdersController extends Controller
{
    ...
    public function index()
    {
       $data = [
        ...
        $admin = Admin::user();
        $user = ShopUser::where('username',$admin->username)->first();
        $id = $user->id;
        $dataTmp = ShopOrder::with('orderTotal')->where('user_id', $id)->where('status', 5)->sort()->paginate(20);
        $dataTr = [];
        foreach ($dataTmp as $key => $row) {  
            $dataTr[] = [
                ...
                'action' => '<a href="' . route('admin_uorders.edit', ['id' => $row['id']]) . '"><span title="' . trans('order.admin.edit') . '" type="button" class="btn btn-flat btn-primary"><i class="fa fa-eye"></i></span></a>'
                ,
            ];
        }
        $data['listTh'] = $listTh;
...
//menu_right
        $data['menu_right'] = '
                        <div class="btn-group pull-right" style="margin-right: 10px">
                           <a href="' . route('admin_uorders.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
...
//menu_sort
      ...
      var url = '" . route('admin_uorders.index') . "?sort_order='+$('#order_sort option:selected').val();
      $.pjax({url: url, container: '#pjax-container'})
    });";
//=menu_sort
//menu_search
        $data['menu_search'] = '
                <form action="' . route('admin_uorders.index') . '" id="button_search">
...
//=menu_search
        $data['url_delete_item'] = route('admin_uorders.delete');
 ...
 */
    public function create()
    {
        $data = [
            ...
            'widget' => [],
            'url_action' => route('admin_uorders.create'),
        ];
 ...
 */
    public function postCreate()
    {
        ...
        return redirect()->route('admin_uorders.index')->with('success', 'Widget created successfully!');
    }
/**
 * Form edit
 */
    ...
    {
        $order = ShopOrder::with('orderTotal')->where('id', $id)->first();
        /*$dnlds = Countdownloads::get();
        print_r($dnlds);*/
        if ($order === null) {
            return 'no data';    
        }
        $data = [
            'title' => 'View Order',
            'sub_title' => '',
            'title_description' => 'View Order',
            'icon' => 'fa fa-pencil-square-o',
            'order' => $order
        ];
        return view('admin.screen.user_order')
            ->with($data);
    }
/**
 * update status
...
//Edit
        ...
        return redirect()->route('admin_uorders.index')->with('success', 'Widget updated successfully!');
    }
/*
Delete list Item
