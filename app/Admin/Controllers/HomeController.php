...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $userid = Admin::user();
        $data = [];
        $data['administrator'] = $user ?? 0;
        $data['title'] = trans('admin.dashboard');
        $data['users'] = new ShopUser;
         if($user)  $data['orders'] = new ShopOrder;
         else $data['orders'] = ShopOrderDetail::where('adminid',$userid->id)->groupBy('order_id')->get();

       
        $data['mapStyleStatus'] = ShopOrder::$mapStyleStatus;

        if($user)  $data['products'] =  new ShopProduct;
        else $data['products'] =  ShopProduct::where('adminid',$userid->id);
...
{
$totals =  ShopOrderDetail::select(
            DB::raw(
                'DATE(created_at) as date,
                DATE_FORMAT(created_at, "%m/%d") as md,
                sum(total_price/exchange_rate) as total_amount,
                count(id) as total_order'
            )
        );
...
if($user){
        ...
        {
            $totalsMonth = ShopOrderDetail::select(
            DB::raw(
                'DATE_FORMAT(created_at, "%Y-%m") as ym,
                        sum(total_price/exchange_rate) as total_amount,
                        count(id) as total_order'
