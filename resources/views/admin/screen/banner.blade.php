                ...
                <div class="box-header with-border">
                    ...
                    <div class="box-tools">
                        <div class="btn-group pull-right" style="margin-right: 5px">
                            <a href="{{ route('admin_banner.index') }}" class="btn  btn-flat btn-default" title="List"><i
                            ...
                            <div class="box-body">
                                <div class="fields-group">
                                    ...
                                    <div class="form-group   {{ $errors->has('image') ? ' has-error' : '' }}">
                                        <label for="image"
                                            class="col-sm-2  control-label">{{ trans('banner.image') }}</label>
                                        <div class="col-sm-8">
                                    ...
                                    <div class="form-group   {{ $errors->has('url_text') ? ' has-error' : '' }}">
                                        <label for="url_text" class="col-sm-2  control-label">Link Text</label>
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <span class="input-group-addon"><i class="fa fa-pencil fa-fw"></i></span>
                                                <input type="text" id="url_text" name="url_text"
                                                    value="{{ old() ? old('url_text') : $banner['url_text'] ?? '' }}"
                                                    class="form-control" placeholder="" />
                                            </div>
                                            @if ($errors->has('url_text'))
                                                <span class="help-block">
                                                    {{ $errors->first('url_text') }}
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group   {{ $errors->has('url') ? ' has-error' : '' }}">
                                        <label for="url"
                                            class="col-sm-2  control-label">{{ trans('banner.url') }}</label>
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <span class="input-group-addon"><i class="fa fa-pencil fa-fw"></i></span>
                                    ...
                                    <div class="form-group  {{ $errors->has('target') ? ' has-error' : '' }}">
                                        <label for="target"
                                            class="col-sm-2 control-label">{{ trans('banner.admin.select_target') }}</label>
                                        <div class="col-sm-8">
                                            <select class="form-control target select2" style="width: 100%;" name="target">
                                                <option value=""></option>
                                                @foreach ($arrTarget as $k => $v)
                                                    <option value="{{ $k }}"
                                                        {{ old('target', $banner['target'] ?? '') == $k ? 'selected' : '' }}>
                                                        {{ $v }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group  {{ $errors->has('html') ? ' has-error' : '' }}">
                                        <label for="html"
                                            class="col-sm-2  control-label">{{ trans('email_template.html') }}</label>
                                        <div class="col-sm-8">
                                            ...
                                            <div class="col-sm-8">
                                                <select class="form-control" name="type">
                                                    @foreach ($dataType as $key => $name)
                                                        <option
                                                            {{ old('type', $banner['type'] ?? '') == $key ? 'selected' : '' }}
                                    ...
                                    <div class="form-group   {{ $errors->has('sort') ? ' has-error' : '' }}">
                                        <label for="sort"
                                            class="col-sm-2  control-label">{{ trans('banner.sort') }}</label>
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <span class="input-group-addon"><i class="fa fa-pencil fa-fw"></i></span>
                                                <input type="number" style="width: 100px;" min=0 id="sort"
                                                    name="sort" value="{{ old() ? old('sort') : $banner['sort'] ?? 0 }}"
                                    ...
                                    <div class="form-group  ">
                                        <label for="status"
                                            class="col-sm-2  control-label">{{ trans('banner.status') }}</label>
                                        <div class="col-sm-8">
                                            <input type="checkbox" name="status"
                                                {{ old('status', empty($banner['status']) ? 0 : 1) ? 'checked' : '' }}>
                                        </div>
                                    </div>
