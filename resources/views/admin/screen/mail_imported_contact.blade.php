                ...
                <div class="box-header with-border">
                    ...
                    <div class="box-tools">
                        <div class="btn-group pull-right" style="margin-right: 5px">
                            <a href="{{ route('admin_customer.index') }}" class="btn  btn-flat btn-default" title="List"><i
                    ...
                    <div class="box-body">
                        <div class="fields-group">
                            ...
                            <div class="form-group  {{ $errors->has('list_id') ? ' has-error' : '' }}">
                                <label for="list_id"
                                    class="col-sm-2 asterisk control-label">Select list</label>
                                <div class="col-sm-8">
                                    <select class="form-control list_id select2" style="width: 100%;" name="list_id">
                                        <option value=""></option>
                                        @foreach ($lists as $list)
                                            <option value="{{ $list->id }}" {{ (old('list_id',$customer['list_id']??'') == $list->id) ? 'selected':'' }}>{{ $list->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group   {{ $errors->has('first_name') ? ' has-error' : '' }}">
                                <label for="first_name" class="col-sm-2 asterisk  control-label">Contact Name</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group   {{ $errors->has('email') ? ' has-error' : '' }}">
                                <label for="email" class="col-sm-2 asterisk  control-label">{{ trans('customer.email') }}</label>
                                <div class="col-sm-8">
                                    <div class="input-group">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        ...
                                        <input type="text" id="state" name="state"
                                            value="{{ old('state', $customer['state'] ?? '') }}" class="form-control state"
                                            placeholder="" />
                                    </div>
                                    @if ($errors->has('state'))
                                        <span class="help-block">
                            ...
                            <div class="form-group  ">
                                <label for="status"
                                    class="col-sm-2  control-label">{{ trans('customer.status') }}</label>
                                <div class="col-sm-8">
                                    <input type="checkbox" name="status"
                                        {{ old('status', empty($customer['status']) ? 0 : 1) ? 'checked' : '' }}>
