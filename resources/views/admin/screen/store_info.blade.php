    ...
    <div class="box box-primary">
      ...
      <tr>
       ...
       <tr>
        <td> Favicon icon
               <span class="input-group-btn">
                 <a data-input="favicon_icon" data-preview="preview1_image" data-type="favicon_icon" class="btn btn-sm btn-flat btn-primary lfm">
                   <i class="fa fa-picture-o"></i> {{trans('product.admin.choose_image')}}
                 </a>
               </span>
        ...
        <td>
            <div class="input-group">
                <input type="hidden" id="favicon_icon" name="favicon_icon" value="{{ $infos->favicon_icon }}" class="form-control input-sm favicon_icon" placeholder=""  />
            </div>
              @if ($errors->has('favicon_icon'))
                  <span class="help-block">
                      {{ $errors->first('favicon_icon') }}
                  </span>
              @endif
            <div id="preview1_image" class="img_holder">{!! sc_image_render($infos->favicon_icon,'100px') !!}</div>
      ...
      <tr>
        <td>Copyright</td>
        <td><a href="#" class="fied-required editable editable-click" data-name="copyright" data-type="text" data-pk="" data-source="" data-url="{{ route('admin_store_info.update') }}" data-title="Copyright" data-value="{{ $infos->copyright }}" data-original-title="" title="">{{$infos->copyright }}</a></td>
      </tr>
      ...
      <tr>
       ...
       <tr>
        <td>Admin email</td>
        <td><a href="#" class="fied-required editable editable-click" data-name="admin_email" data-type="text" data-pk="" data-source="" data-url="{{ route('admin_store_info.update') }}" data-title="Admin email" data-value="{{ $infos->admin_email }}" data-original-title="" title="">{{$infos->admin_email }}</a></td>
      </tr>
...
$('[name="favicon_icon"]').change(function(event) {
      var APP_URL = {!! json_encode(url('/')) !!}
    APP_URL = APP_URL;
    var replit = $('#favicon_icon').val().replace(APP_URL,"");
    $('#favicon_icon').val(replit);
          $.ajax({
        url: '{{ route('admin_store_info.update') }}',
        type: 'POST',
        dataType: 'JSON',
        data: {"name": 'favicon_icon',"value":$(this).val(),"_token": "{{ csrf_token() }}",},
      })
      .done(function(data) {
        if(data.stt == 1){
          const Toast = Swal.mixin({
