...
@section('main')
<div class="cart-wrapsss">
<div class="container">
 <div class="col-md-12 width_int">
    <div class="row g-0 product-single-main">

@if (count($cart) ==0)  
    <div class="col-md-12 cart-col cart-checkout-inner width_int"><div class="cart-empty"><h2>Cart is empty</h2><a href="{{ route('product.all') }}">Back to Shop</a></div> </div>
@else
 <center><h1 class="title">{{ $title }}</h1></center>
<div class="table-responsive view_cart">
<table class="table box table-bordered">
 ...
 <?php $free = 1; ?>
    @foreach($cart as $item)
        ...
        @endphp
         <?php if($product->noprice==0) $free=0; ?>
    <tr class="row_cart">
        <td >{{ $n }}</td>
...
{{-- //end Process attributes --}}
        ...
        <td data-title="Product Price">{!! $product->showPrice() !!}</td>
        <td data-title="Product Quantity"><input style="width: 70px;" type="number" data-id="{{ $item->id }}" data-rowid="{{$item->rowId}}" onChange="updateCart($(this));" class="item-qty" name="qty-{{$item->id}}" value="{{$item->qty}}"><span class="text-danger item-qty-{{$item->id}}" style="display: none;"></span></td>
        <td align="right" data-title="Product Subtotal">{{sc_currency_render($item->subtotal)}}</td>
        <td>
            <a onClick="return confirm('Are you sure?')" title="Remove Item" alt="Remove Item" class="cart_quantity_delete" href="{{route("cart.remove",['id'=>$item->rowId,'pid'=>$item->id])}}"><i class="fa fa-trash-o"></i></a>
        </td>
    </tr>
    @endforeach
    </tbody>
        ...
        <tr  style="background: #eaebec">
            <td colspan="7">
                ...
                <button class="btn btn-default" type="button" style="cursor: pointer;padding:10px 30px" onClick="location.href='{{ route('home') }}'"><i class="fa fa-arrow-left"></i> {{ trans('cart.back_to_shop') }}</button>
                 ...
                 <div class="pull-right clear-cart">
...
{{-- Coupon --}}
        ...
        <?php if($free==0) { ?>
                 <div class="row coupon-sec {{ Session::has('Discount')? 'coupon-exist' :'' }} ">
                  <div class="form-group col-md-12">
                    <label class="control-label" for="inputGroupSuccess3"> 
                        <span style="display:inline; cursor: pointer; display: {{ ($hasCoupon)?'block':'none' }}" class="text-danger" id="removeCoupon">{{ Session::has('Discount')? Session('Discount') :'' }} <i class="fa fa fa-times"></i></span>
                    </label>
                        ...
                        @foreach ($shippingMethod as $key => $shipping)
                            <div>
                                <label class="radio-inline">
                                 <input type="radio" name="shippingMethod" value="{{ $shipping['code'] }}"  {{ (old('shippingMethod') == $key)?'checked':'' }} style="position: relative;" {{ ($shipping['permission'])?'':'disabled' }} checked>
            ...
            <div class="col-md-12">
                    ...
                    <div class="form-group">
                         <?php if($free==1) { ?>
                            <div>  
                            <label class="radio-inline">
                             <input type="radio" name="paymentMethod" value="Cash" style="position: relative;">
                                <img title="Free" alt="Free" src="{{ asset('images/free.png') }} " style="width: 120px;">
                            </label>
                        </div>
                      <?php  } else { ?>
                        @foreach ($paymentMethod as $key => $payment)
                         <?php if($payment['code']!='Cash') { ?>
                             <div>  
                                <label class="radio-inline">
							...
							<div class="paypal-notes">paying with Paypal will require you to select<br><span>[Debit or Credit Card]</span><br>When you login to your Paypal. Pls select that when prompted by Paypal.</div>
                             <?php } ?> 
                        @endforeach
                         <div class="payment-inner">
                                <label class="radio-inline">
                                 <input type="radio" name="paymentMethod" value="stripe"  {{ (old('paymentMethod') == 'stripe')?'checked':'' }} style="position: relative;" >
                             <img alt="Payment Icon" src="{{ asset('/images/mastercard.png') }} " />                              
...
@if ($extensionDiscount)
        ...
        setTimeout(function() {
            ...
            .done(function(result) {
                ...
                if(result.error ==1){
                    ...
                    $('#removeCoupon').html(coupon+'<i class="fa fa fa-times"></i>');
                    $('.coupon-sec').addClass('coupon-exist');
                    $('#removeCoupon').show();
    ...
    $('#removeCoupon').click(function() {
            ...
            .done(function(result) {
                    $('#removeCoupon').hide();
                    $('.coupon-sec').removeClass('coupon-exist');
                    $('#coupon-value').val('');
                    $('.coupon-msg').removeClass('text-danger');
                    $('.coupon-msg').removeClass('text-success');
                    $('.coupon-msg').hide();
